...
package PostClass_m;
# 이것의 하나의 object는 PostClass의 것과 다릅니다. 왜냐하면 이것은 한 object에 댓글과 트랙백 정보 모두를 저장하고 있습니다.
...
#		유니코드 0x25CF는 점을 뜻한다.
		...
		# 변수 할당.
		$title = BackUpEgloos_Subs::findstr($content_html, '<div class="subject"><h3>(?:<img src[^>]+> )?', '<\/h3>');
...
#		트랙백, 댓글 개수
		$trackback_count = $open_close{comment_cnt};
		$comment_count = $open_close{trackback_cnt};
		
#		postid로 디렉토리 만들기. - 있는 경우 처리.
		if(!(-e './data/mobile/' . $postid))
		{
			mkdir('./data/mobile/' . $postid) or die "폴더 만들기 에러.\n";
		}
...
#		트랙백과 댓글 파일도 다운로드 받는다.
		...
		my $comment_count_div_10 = ceil($comment_count / 10);
		for($idx = 1 ; $idx <= $comment_count_div_10 ; $idx++)
		{
			# 댓글 받기
			$filename_temp = 'data/mobile/' . $postid . '/comment_list_' . $idx . '.xml';
			# 파일이 존재하지 않는다면 여기에 쓰기
			if(!(-e $filename_temp))
			{
				$content_html_temp = BackUpEgloos_Subs::getpage($egloosinfo->{blogurl} . '/m/comment/' . $postid . '/page/' . $idx, 0);
				open(OUT, ">:encoding(utf8) " ,$filename_temp) or die $!;
				print OUT $content_html_temp;
				close(OUT);
			}
...
#		이미지 저장할 경로 설정.
		...
		my $img_dest = 'data/mobile/' . $postid . '/' . $istr . '.' . $img_extension;
#		다운로드.
